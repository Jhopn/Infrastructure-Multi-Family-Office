...
import { createNetWorthSnapshot, deleteNetWorthSnapshot, getNetWorthSnapshots } from 'controllers/net-worth-controller/net-worth-controller';
...
const NetWorthRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.post('/net-worth', {
        preHandler: authAccess(["User"]),
        schema: {
            description: 'Create a new net worth snapshot for the authenticated client',
            tags: ['Net Worth'],
    ...
    fastify.get('/net-worth', {
        preHandler: authAccess(["User"]),
        schema: {
            description: 'Get net worth snapshots for the authenticated client. Can be filtered by date.',
            tags: ['Net Worth'],
