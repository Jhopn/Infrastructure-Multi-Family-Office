...
const RetirementProfileRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.post('/retirement-profile', {
        preHandler: authAccess(["User"]),
        schema: {
            tags: ['Retirement Profile'],
            security: [{ bearerAuth: [] }],
            description: 'Create the retirement profile for the authenticated client. Fails if one already exists.',
            body: createRetirementProfileSchema,
        }
    }, createRetirementProfile);

    fastify.get('/retirement-profile', {
        preHandler: authAccess(["User"]),
        schema: {
            tags: ['Retirement Profile'],
            security: [{ bearerAuth: [] }],
    ...
    fastify.patch('/retirement-profile', {
        preHandler: authAccess(["User"]),
        schema: {
            tags: ['Retirement Profile'],
            security: [{ bearerAuth: [] }],
            description: 'Update the retirement profile for the authenticated client.',
            body: updateRetirementProfileSchema,
        },
    }, updateRetirementProfile);

    fastify.delete('/retirement-profile', {
        preHandler: authAccess(["User"]),
        schema: {
            tags: ['Retirement Profile'],
            security: [{ bearerAuth: [] }],
            description: 'Delete the retirement profile for the authenticated client.',
        }
    }, deleteRetirementProfile);
};
