...
export const createRetirementProfileSchema = z.object({
    ...
    retirementAge: z.number().int().positive().max(MAX_INT4, `Retirement age is too high.`).optional(),
    currentContribution: z.number().min(0).optional(),
});

export const updateRetirementProfileSchema = createRetirementProfileSchema.partial();
