...
export const createGoal = async (request: FastifyRequest<{ Body: z.infer<typeof createGoalSchema> }>, reply: FastifyReply) => {
    try {
...
export const updateGoal = async (request: FastifyRequest<{ Body: z.infer<typeof updateGoalSchema>, Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        ...
        if (existingGoal.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only update your own goals." });
        }
...
export const deleteGoal = async (request: FastifyRequest<{ Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        const { id } = request.params;
        ...
        if (existingGoal.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only delete your own goals." });
        }
