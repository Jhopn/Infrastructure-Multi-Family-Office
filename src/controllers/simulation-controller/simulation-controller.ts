...
import type { createSimulationSchema } from './dto/simulation.dto';

export const createSimulation = async (
  request: FastifyRequest<{ Body: z.infer<typeof createSimulationSchema> }>,
  reply: FastifyReply,
) => {
  try {
    ...
    const { label, rate, startDate, initialValue, monthlyContribution, years } =
      request.body;
  ...
  try {
    ...
    if (!simulation) {
      return reply.code(404).send({
        error: 'Simulation not found or you do not have permission to view it.',
      });
    }
