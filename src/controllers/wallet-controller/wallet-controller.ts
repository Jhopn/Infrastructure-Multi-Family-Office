...
import type { createWalletSchema, updateWalletSchema } from './dto/wallet.dto';

export const createWalletItem = async (
  request: FastifyRequest<{ Body: z.infer<typeof createWalletSchema> }>,
  reply: FastifyReply,
) => {
  try {
...
export const updateWalletItem = async (
  request: FastifyRequest<{
    Body: z.infer<typeof updateWalletSchema>;
  ...
  try {
    ...
    if (existingItem.clientId !== request.clientData!.id) {
      return reply.code(403).send({
        error: 'Forbidden: You can only update your own wallet items.',
      });
    }
...
export const deleteWalletItem = async (
  ...
  try {
    ...
    if (existingItem.clientId !== request.clientData!.id) {
      return reply.code(403).send({
        error: 'Forbidden: You can only delete your own wallet items.',
      });
    }
