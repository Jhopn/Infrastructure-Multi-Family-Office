...
export const createInsurance = async (request: FastifyRequest<{ Body: z.infer<typeof createInsuranceSchema> }>, reply: FastifyReply) => {
    try {
...
export const getInsurances = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
...
export const updateInsurance = async (request: FastifyRequest<{ Body: z.infer<typeof updateInsuranceSchema>, Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        ...
        if (existingInsurance.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only update your own insurances." });
        }
...
export const deleteInsurance = async (request: FastifyRequest<{ Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        const { id } = request.params;
        ...
        if (existingInsurance.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only delete your own insurances." });
        }
