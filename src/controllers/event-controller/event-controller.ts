...
export const createEvent = async (request: FastifyRequest<{ Body: z.infer<typeof createEventSchema> }>, reply: FastifyReply) => {
    try {
...
export const updateEvent = async (request: FastifyRequest<{ Body: z.infer<typeof updateEventSchema>, Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        ...
        if (existingEvent.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only update your own events." });
        }
...
export const deleteEvent = async (request: FastifyRequest<{ Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        const { id } = request.params;
        ...
        if (existingEvent.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only delete your own events." });
        }
