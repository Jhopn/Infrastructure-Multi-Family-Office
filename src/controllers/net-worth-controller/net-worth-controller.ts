...
} from './dto/net-worth.dto';

export const createNetWorthSnapshot = async (
  request: FastifyRequest<{
    Body: z.infer<typeof createNetWorthSnapshotSchema>;
  }>,
  reply: FastifyReply,
) => {
  try {
    ...
    return reply
      .code(400)
      .send({ error: 'Error creating net worth snapshot.' });
  }
};

export const getNetWorthSnapshots = async (
  request: FastifyRequest<{
    Querystring: z.infer<typeof getNetWorthQuerySchema>;
  }>,
  reply: FastifyReply,
) => {
  try {
    ...
    return reply
      .code(400)
      .send({ error: 'Error fetching net worth snapshots.' });
  }
};
...
export const deleteNetWorthSnapshot = async (
  ...
  try {
    ...
    if (existingSnapshot.clientId !== clientId) {
      return reply
        .code(403)
        .send({ error: 'Forbidden: You can only delete your own snapshots.' });
    }
    ...
    return reply
      .code(400)
      .send({ error: 'Error deleting net worth snapshot.' });
  }
};
