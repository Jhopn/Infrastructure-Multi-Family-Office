...
export const createIdealWalletItem = async (request: FastifyRequest<{ Body: z.infer<typeof createIdealWalletSchema> }>, reply: FastifyReply) => {
    try {
...
export const getIdealWalletItems = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
...
export const updateIdealWalletItem = async (request: FastifyRequest<{ Body: z.infer<typeof updateIdealWalletSchema>, Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        ...
        if (existingItem.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only update your own ideal wallet items." });
        }
...
export const deleteIdealWalletItem = async (request: FastifyRequest<{ Params: z.infer<typeof uuidParamSchema> }>, reply: FastifyReply) => {
    try {
        const { id } = request.params;
        ...
        if (existingItem.clientId !== request.clientData!.id) {
            return reply.code(403).send({ error: "Forbidden: You can only delete your own ideal wallet items." });
        }
